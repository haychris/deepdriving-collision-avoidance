////////////////////////////////////////////////
//
//  Drive a car in TORCS with one-lane scenario.
//  Used with TORCS driver agent "chenyi" (for one-lane), and modified tracks with one-lane pavement textures.
//  Must configure other traffic cars (chenyi_AIx) to drive in one-lane setting before run the code.
//
//  Input keys 
//  Esc: exit
...
int main(int argc, char** argv) {
    ...
    IplImage* background=cvLoadImage("../torcs/semantic_background_1lane.png");
    ...
    while (1) {
        ...
        if (shared->written == 1) {  // the new image data is ready to be read
            ...
            if (shared->control==1) {
               ...
               if (coe_steer>1 && shared->steerCmd>0.1)   // reshape the steering control curve
