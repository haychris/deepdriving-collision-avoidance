...
initStartingGrid(void)
{
	...
	/////// Added by Chris
	tdble CAR_OFFSET = 0; // offset automatically added by the game
	tdble car_0_speed = rand() % 130; //150
	tdble car_1_speed = rand() % 130; //150
	...
	tdble desired_offset = rand() % max_desired_offset + min_desired_offset;
	// int useBrakes = rand() % 2;
	// int useSteering = rand() % 2;
	tdble useBrakes = rand() / (RAND_MAX + 1.);
	tdble useSteering = rand() / (RAND_MAX + 1.);
	std::cout << "car_0_speed: " << car_0_speed << "\n";
	std::cout << "car_1_speed: " << car_1_speed << "\n";
	std::cout << "desired_offset: " << desired_offset << "\n";
	...
	for (i = 0; i < ReInfo->s->_ncars; i++) {
		...
		// int car_1_speed = 60;
		// int car_2_speed = 100;
		// int desired_offset = 5;
		if (ReInfo->s->_ncars == 2){
			if (i == 0) {
				car->_speed_x = car_0_speed;
				startpos = ReInfo->track->length - 1.0*car->_speed_x;
			}
			else {
				car->_speed_x = car_1_speed;
				startpos = ReInfo->track->length - 1.0*(car->_speed_x + desired_offset - CAR_OFFSET);
		...
		// int counter = 0;
		// int num_segs_back = 10;
		// if (ReInfo->s->_ncars == 2) {
		// 	if (i == 1) {
		// 		while (counter < num_segs_back) {
		// 			curseg = curseg->prev;
		// 		}
		// 	}
		// } else {
