...
int printed_initial_speeds = 0;
bool went_offroad = FALSE;
...
/////////////////////////// by Chenyi, output 1 image per 0.1 second
     if (*ppause == 1) 
     { 
        count++;
        if (count>100)
...
/////////////////////////// by Chenyi, output 1 image per 0.1 second
	...
	if ((ReInfo->_displayMode != RM_DISP_MODE_NONE) && (ReInfo->_displayMode != RM_DISP_MODE_CONSOLE)) {
		if (floor(s->currentTime) == -1.0) {
			ReRaceBigMsgSet("Ready", 0.5);
		} else if (floor(s->currentTime) == -0.5) {
			ReRaceBigMsgSet("Set", 0.5);
		} else if (floor(s->currentTime) == 0.0) {
			ReRaceBigMsgSet("Go", 0.5);
		}
	}

	///// Chris
	tCarElt* obstacleCarAhead;
	tCarElt* curCar;
	...
	if (s->_ncars == 2 && !printed_initial_speeds && s->currentTime > 0.001) {
		...
		dataFile << "starting_distance: " << obstacleCarAhead->_distFromStartLine - curCar->_distFromStartLine << "\n";
		...
		dataFile << "obstacleCarAhead starting_damage: " << obstacleCarAhead->priv.dammage << "\n"; 
  		dataFile << "curCar starting_damage: " << curCar->priv.dammage << "\n";
		printed_initial_speeds = 1;
	}
	if (s->_ncars == 2 && !went_offroad) {
		went_offroad = abs(curCar->_trkPos.toMiddle) > 6.0;
	}
	// std::cout << curCar->_pos_X << " " << curCar->_pos_Y<< " " << went_offroad<<"\n";
	// std::cout << obstacleCarAhead->info.startRank << " " << curCar->info.startRank  << "\n"   ;
	if (printed_initial_speeds) {
		double curAccel = sqrt(curCar->_accel_x*curCar->_accel_x + curCar->_accel_y*curCar->_accel_y + curCar->_accel_z*curCar->_accel_z);
	...
	if (curCar->_pos_X != curCar->_pos_X)
		restartRequested = true;
	// std::cout << 2*(obstacleCarAhead->_pos_X > curCar->_pos_X) -1 << "\n"   ;
	ReInfo->_reCurTime += deltaTimeIncrement * ReInfo->_reTimeMult; /* "Real" time */
	s->currentTime += deltaTimeIncrement; /* Simulated time */
	   
    if(restartRequested){  
      	if (s->_ncars == 2) {
      		dataFile << "obstacleCarAhead final_damage: " << obstacleCarAhead->priv.dammage << "\n"; 
  			dataFile << "curCar final_damage: " << curCar->priv.dammage << "\n";
  			dataFile << "obstacleCarAhead final_speed: " << obstacleCarAhead->pub.speed << "\n";
			dataFile << "curCar final_speed: " << curCar->pub.speed << "\n";

			double final_distance = obstacleCarAhead->_trkPos.seg->lgfromstart + getDistToSegStart(obstacleCarAhead) - curCar->_distFromStartLine;
    	    if (final_distance > ReInfo->track->length/2.0f) {
    	        final_distance -= ReInfo->track->length;
	        } else if (final_distance < -ReInfo->track->length/2.0f) {
	            final_distance += ReInfo->track->length;
	        }
	        // final_distance *= curCar->race.pos -  obstacleCarAhead->race.pos;
			dataFile << "final_distance: " << final_distance << "\n";
  			dataFile << "curCar went_offroad: " << went_offroad << "\n";
  			dataFile << "curCar maxAccel: " << maxAccel << "\n";
  			dataFile << "curCar avgAccel: " << sumAccel / updateCount << "\n";
  			
  			dataFile << ".\n";
  			dataFile.close(); 
      	}
      	printed_initial_speeds = 0;
      	went_offroad = 0;
    ...
    double speedRestartThreshold = 0.1;
    double minTime = 1;
    double maxTime = 15;
    if (s->_ncars == 2 && s->currentTime > minTime){
    	if (s->cars[1]->pub.speed < speedRestartThreshold ||  s->currentTime > maxTime) 
    		restartRequested = true;
    }
	// if (s->currentTime > 10) {
